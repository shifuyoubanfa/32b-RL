"""本地打分器（RL 各阶段共用）—— 自 14B 沙盒移植，逻辑已验证（humanness 0.21→0.846）。

设计原则：
- think 自然度：纯规则——RAG 痕迹词密度 + 与参考资料的照抄率（LCS + 5-gram）。
- answer 正确性：不判对错，改判"答案漂移"——与 V1 金标准的相似度 + 关键事实召回。
- 零重依赖：基础项只用 re / difflib；PMI 需外部传入一个"尺子模型"前向打分。

reward 组合（乘法门控）：
- 格式不合法            -> reward = -1.0
- 答案漂移(R_acc<τ)    -> reward = 0.1 * R_acc
- 否则                 -> reward = R_acc * (w_acc + w_human * R_human)

【32B 变化】本轮打开 PMI：humanness 融合条件 PMI 结构信号。
关键：`pmi_cond(model, tok, ...)` 的 model 是"度量尺子"，不是被奖励对象——
由探针 AUC 选优（候选：当前模型 / 冻结 πref / 干净 Qwen 底座，排除原始 V1）。
"""

import re
from difflib import SequenceMatcher

# ---- RAG 痕迹正则（匹配学生实际写法，放宽不要求"参考问答对"后紧跟数字）----
_TRACE_PATTERNS = [
    r"参考问答对",
...
def parse_think_answer(text: str) -> tuple[str, str]:
    """解析生成文本里的 <think>...</think> 与 <answer>...</answer>。

    V1(Qwen2.5) 不靠模板注入开 <think>，而是模型自发吐完整 <think>...</think><answer>...</answer>；
    本函数用 .replace('<think>','') 兜底，对"自发吐"与"模板注入"两种都鲁棒（已实测验证）。
    """
    text = text or ""
...
def parse_think_answer_diagnostic(text: str) -> dict:
    """Parse one generation without hiding a truncated/malformed response.

    The legacy ``parse_think_answer`` fallback intentionally remains unchanged for
    old training/data-building callers.  Evaluation uses this diagnostic parser so
    a max-token truncation inside ``<think>`` is accounted as model output:
    preserve the partial think, keep the answer empty, and mark a format failure.
    No text is generated or copied from the gold/V1 answer here.
    """
    raw = text or ""
    ...
    if close != -1:
        ...
        # The response never left its reasoning section.  Retain that observed
        # partial reasoning, but never let the legacy fallback masquerade it as
        # an answer.
...
def copy_signal(think: str, references: str) -> float:
    """照抄强度 = max(最长公共子串占比, 5-gram 重叠率)。LCS 抓连续照搬，n-gram 抓打散/改写式照搬。"""
    t = (think or "")[:3000]
...
def humanness(think: str, references: str, c_trace: float = None, c_copy: float = None,
    ...
    可选 PMI(结构信号,需外部用尺子模型算好传入 s_pmi=-PMI)：score = w_pmi·sigmoid(2·s_pmi) + (1-w_pmi)·base。
    """
    ...
    copy_ratio = copy_signal(think, references)
...
def answer_drift(gen_answer: str, gold_answer: str) -> tuple[float, dict]:
    """答案没漂移=高分：与 V1 金标准答案的相似度 + 关键事实召回。不是判对错，是判'变没变'。"""
...
def pmi_cond(model, tok, system_prompt: str, qr_user: str, qa_user: str,
    ...
    只前向打分、不生成；同一 think_ids 在两上下文各算一次。
    `model` 是"度量尺子"（探针 AUC 选优出的：当前模型 / 冻结 πref / 干净 Qwen 底座），由调用方传入并冻结。
    """
    import torch
    think = (think or "").strip()
    if not think:
...
def score_rollout(gen_text: str, user_prompt: str, gold_answer: str, *,
                  tau_acc: float = 0.30, w_human: float = 0.5, w_acc: float = 0.5,
